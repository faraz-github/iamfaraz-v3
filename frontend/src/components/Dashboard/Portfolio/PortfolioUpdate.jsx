...
const PortfolioUpdate = ({ token, selectedPortfolio, setTabIndex }) => {
  // context
  const { setOpenLoading } = useLoading();

  // state
  const [name, setName] = useState(selectedPortfolio?.name);
  const [type, setType] = useState(selectedPortfolio?.type);
  const [description, setDescription] = useState(
    selectedPortfolio?.description
  );
  const [inputStack, setInputStack] = useState("");
  const [stack, setStack] = useState(selectedPortfolio?.stack);
  const [link, setLink] = useState(selectedPortfolio?.link);
  const [source, setSource] = useState(selectedPortfolio?.source);

  const [coverPicture, setCoverPicture] = useState(selectedPortfolio?.picture);
  const [firstScreenPicture, setFirstScreenPicture] = useState(
    selectedPortfolio?.firstScreen
  );
  const [secondScreenPicture, setSecondScreenPicture] = useState(
    selectedPortfolio?.secondScreen
  );
  const [lastScreenPicture, setLastScreenPicture] = useState(
    selectedPortfolio?.lastScreen
  );
  ...
  const onUpdateHandler = async (event) => {
    ...
    if (
      ...
      if (!coverPicture) {
        ...
        const id = selectedPortfolio._id;
  ...
  return (
    <Card variant="outlined" square>
      <CardHeader title="Portfolio Details" subheader="Update" />
